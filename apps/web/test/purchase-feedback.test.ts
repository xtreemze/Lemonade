...
describe("purchase feedback schedule", () => {
  it("schedules serve, payment and drink in order for every small-day sale", () => {
    const schedule = createPurchaseFeedbackSchedule(5, 5_000);
    ...
    for (const beat of schedule) {
      ...
      expect(beat.drinkAtMs).toBeLessThanOrEqual(5_000);
    }
  });

  it("rate-limits dense days while sampling the full sales span", () => {
    const schedule = createPurchaseFeedbackSchedule(400, 6_000);
  ...
  it("is deterministic and produces no events when nothing sold", () => {
    expect(createPurchaseFeedbackSchedule(0, 5_000)).toEqual([]);
    expect(createPurchaseFeedbackSchedule(12, 5_000)).toEqual(
      createPurchaseFeedbackSchedule(12, 5_000),
    );
  });
});

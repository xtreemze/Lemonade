...
const applyStripTransform = (node: TransformNode, strip: StreetStripSpec): void => {
  node.position.set(strip.x, strip.role === "sidewalk" ? 0.022 : 0.012, strip.z);
  node.rotation.y = -strip.rotationY;
  node.scaling.set(strip.length, 0.022, strip.width);
};

const createAnchor = (scene: Scene, strip: StreetStripSpec): TransformNode => {
...
export const createBabylonStreetSurfaceField = (
  ...
  const batches = [
    ...
    createBatch(scene, "sidewalk", "SidewalkSurfaces", sidewalks, 0xd4d0c6),
  ].filter((batch): batch is BabylonStreetSurfaceBatch => batch !== null);

  return Object.freeze({
    anchors: Object.freeze([...roads, ...sidewalks].map((strip) => createAnchor(scene, strip))),

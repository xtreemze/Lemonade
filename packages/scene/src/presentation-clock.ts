...
export const clampPresentationElapsed = (
  elapsedMs: number,
  durationMs: number,
): number =>
  Math.min(
...
export const animationElapsedAt = (
  ...
  clampPresentationElapsed(timestampMs - epochMs, durationMs);
...
export const stateUpdateElapsed = (
  ...
  presentationChanged
    ? 0
    : clampPresentationElapsed(lastElapsedMs, durationMs);

export const resumedAnimationEpoch = (
  timestampMs: number,
  lastElapsedMs: number,
): number =>

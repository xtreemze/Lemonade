...
describe("environment presentation contract", () => {
  it("produces the same semantic frame for the same weather and elapsed time", () => {
    const first = environmentPresentationFrameAt("thunderstorm", "simulation", 5600, 14_000, false);
  ...
  it("uses one thunder occurrence schedule for visual, audio and haptic consumers", () => {
    const schedule = environmentOccurrenceSchedule("thunderstorm", "simulation", 14_000);
    const thunder = schedule.filter((occurrence) => occurrence.kind === "thunder");

    expect(thunder.map((occurrence) => occurrence.atMs)).toEqual([2800, 7980, 10_920]);
  ...
  it("only schedules birdsong for sunny simulation presentation", () => {
    ...
    ).toEqual([1400, 4300, 7200]);

    expect(
      environmentOccurrenceSchedule("sunny", "forecast", 6000).some(
  ...
  it("keeps semantic occurrence order independent of frame subdivision", () => {
    const whole = environmentOccurrencesBetween("thunderstorm", "simulation", 14_000, 0, 14_000);
    const split = [
      ...environmentOccurrencesBetween("thunderstorm", "simulation", 14_000, 0, 5000),
      ...environmentOccurrencesBetween("thunderstorm", "simulation", 14_000, 5000, 9000),
      ...environmentOccurrencesBetween("thunderstorm", "simulation", 14_000, 9000, 14_000),
    ];
  ...
  it("reduced motion changes motion output without changing semantic weather events", () => {
    ...
    expect(environmentOccurrenceSchedule("thunderstorm", "simulation", 14_000)).toEqual(
  ...
  it("forecast and simulation use explicit business-day timeline modes", () => {
    const forecast = environmentPresentationFrameAt("sunny", "forecast", 4000, 6000, false);
    const simulation = environmentPresentationFrameAt("sunny", "simulation", 4000, 14_000, false);

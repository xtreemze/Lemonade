...
describe("semantic neighborhood navigation", () => {
  it("is deterministic and follows the seeded residential plan", () => {
    const layout = generateResidentialLayout(0x1234abcd);
  ...
  it("keeps sidewalk nodes inside their semantic sidewalk bands", () => {
    const graph = createNeighborhoodNavigationGraph(
      generateResidentialLayout(77),
    );

    for (const node of graph.nodes) {
      if (node.role !== "sidewalk" || node.side === null) continue;
      const sidewalk =
        node.side === "near"
  ...
  it("routes far-side pedestrians through a real crossing before stand access", () => {
    const graph = createNeighborhoodNavigationGraph(
      generateResidentialLayout(91),
    );
    const farNodes = graph.nodes
      .filter((node) => node.role === "sidewalk" && node.side === "far")
      .sort((left, right) => left.x - right.x);
    const start = farNodes[0];
    if (start === undefined) throw new Error("expected far sidewalk node");

    const path = shortestNavigationPath(
      graph,
      start.id,

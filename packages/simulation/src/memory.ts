...
const NEUTRAL_SATISFACTION_BPS = 5_000;
const MIN_SATISFACTION_TARGET_BPS = 3_500;
const MAX_SATISFACTION_TARGET_BPS = 6_500;
...
const assertObservation = (observation: MarketMemoryObservation): void => {
  ...
  if (!Number.isSafeInteger(purchased) || purchased < 0) {
    throw new RangeError(
      "purchased customers must be a non-negative safe integer",
    );
  }
...
const nextExpectedPrice = (
  current: MoneyCents,
  observed: MoneyCents,
): MoneyCents => {
  if (Number(current) === 0) return observed;
  return moneyCents(
    Math.round(
...
const satisfactionTarget = (
  ...
  if (willing === 0) return NEUTRAL_SATISFACTION_BPS;

  const fulfillment = purchased / willing;
  return (
    MIN_SATISFACTION_TARGET_BPS +
    fulfillment *
      (MAX_SATISFACTION_TARGET_BPS - MIN_SATISFACTION_TARGET_BPS)
  );
};
...
export const deriveMarketMemory = (
  history: readonly MarketMemoryObservation[],
): MarketMemory => {
